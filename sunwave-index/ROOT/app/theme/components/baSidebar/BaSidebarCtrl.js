(function() {
    ...
    function BaSidebarCtrl($rootScope, $scope, baSidebarService, $http, $window) {
        ...
        $scope.getStateRef = function($event, item) {
            ...
            if (item.level == 0) {
                if (item.id == '1300' || item.id == '1600' || item.id == '1') {
                    $rootScope.menuTitle = item.title;
                } else if ($rootScope.menuTitle == undefined) {
                    $rootScope.menuTitle = '';
                }
            } else {
                $rootScope.menuTitle = item.title;
            ...
            // $rootScope.menuTitle = item.title;
            //chinese title
            $rootScope.menuTitle_chinese = item.title_chinese;
            window.sessionStorage.setItem('menuId', $rootScope.menuId);
            window.sessionStorage.setItem('menuTitle', $rootScope.menuTitle);
            window.sessionStorage.setItem('menuTitle_chinese', $rootScope.menuTitle_chinese);
