(function() {
    ...
    angular.module('SunWave.pages.AlarmConfiguration.SNMPConfiguration', [])
        .controller('SNMPConfigurationCtrl', SNMPConfigurationCtrl);

    function SNMPConfigurationCtrl($scope, SNMPConfigurationService, $uibModal, $log, $http, messageAlertService) {
        $scope.checkbox = {
            select_all: "",
        }
        ...
        $scope.selectOne = function() {
            angular.forEach($scope.items, function(i) {
                ...
                var index = $scope.checked.indexOf(i.trapId);
        ...
        $scope.deleteItem = function() {
            if ($scope.checked.length == 0) {
                ...
                if (messageAlertService.confirmFun('sure')) {
                    // //console.log("选中项");
                    //console.log($scope.checkedItems);
                    SNMPConfigurationService.deleteItem($scope.checked).success(function(response) {
        ...
        var searchForm = function() {
            ...
            $scope.clearAll();
            SNMPConfigurationService.searchContent($scope.query)
        ...
        $scope.openEditDialog = function() {
            var modalInstance = $uibModal.open({
                animation: true,
                backdrop: "static",
                templateUrl: 'app/pages/AlarmConfiguration/SNMPConfiguration/addModal.html',
                controller: 'addCtrl',
                size: 'lg',
                resolve: {
                    modal: function() {
                    ...
                    SNMPConfigurationService: function() {
                        return SNMPConfigurationService;
                    },
                    deps: ['$ocLazyLoad',
                        function($ocLazyLoad) {
                            return $ocLazyLoad.load(['app/pages/AlarmConfiguration/SNMPConfiguration/addModalCtrl.js']);
                        }
                    ]
                }
            });
