...
function detailDeviceNameModalCtrl($rootScope, $scope, deviceListService, topService, checkedItems, $uibModalInstance, messageAlertService, $uibModal) {
    ...
    var noClickFun = function() {
        let noClick_typeid = $scope.checkedItems.neDevicetype.neDevicetypeclassify.dtcDevicetypeclassifyid;
            ...
            .success(function(response) {
                ...
                for (var i = 0; i < $scope.rowsQS.length; i++) {
                    ...
                    if ($scope.rowsQS[i].name == 'Device ID') {
                        ...
                        // $scope.rowsQS[i].value = ($scope.rowsQS[i].value).toString(16).toUpperCase();
                        $scope.rowsQS[i].value = checkedItems.neRepeaterid16;
        ...
        deviceListService.obtainAlarmInfo(checkedItems.neNeid)
            .success(function(response) {
                ...
                for (let i = 0; i < $scope.al_rows.length; i++) {
                    if ($scope.al_rows[i].objValueSet) {
                        $scope.al_rows[i].isSelec = true;

                    } else {
                        $scope.al_rows[i].isSelec = false;
                    }
                }
    ...
    $scope.getWebSocketMsg = function(winId, winFlag, queryFlag) {
        $rootScope.websocket.onmessage = function(event) {
            // //console.log("收到的----------------" + event + "-----" + new Date());
            // $scope.websoketData = $rootScope.webSocketDeviceParamsData;

            // $scope.$on('toDeviceDetailMsg', function(event, data) {
            //console.log("收到的----------------" + event.data + "-----" + new Date());
            $scope.websoketData = event.data;
            ...
            for (let i = 0; i < $scope.websoketData.length; i++) {
                ...
                $scope.detail = $scope.websoketData[i].detail;
                        ...
                        if ($scope.rowsCOM.length != 0) {
                            ...
                            for (let k = 0; k < $scope.rowsCOM.length; k++) {
                                if ($scope.commid == $scope.rowsCOM[k].commid) {
                                    ...
                                    $scope.rowsCOM[k].detail = $scope.detail;
                        ...
                        if ($scope.al_COM_rows.length != 0) {
                            ...
                            for (let k = 0; k < $scope.al_COM_rows.length; k++) {
                                ...
                                if ($scope.commid == $scope.al_COM_rows[k].commid) {
                                    ...
                                    $scope.al_COM_rows[k].detail = $scope.detail;
                        ...
                        if ($scope.ra_rowsCOM.length != 0) {
                            ...
                            for (let k = 0; k < $scope.ra_rowsCOM.length; k++) {
                                ...
                                if ($scope.commid == $scope.ra_rowsCOM[k].commid) {
                                    ...
                                    $scope.ra_rowsCOM[k].detail = $scope.detail;
    ...
    $scope.detailShow = function(item, event) {
        var modalInstance = $uibModal.open({
            animation: true,
            backdrop: "static",
            templateUrl: 'app/pages/deviceManagement/deviceList/detailShow.html',
                ...
                deps: ['$ocLazyLoad',
                    function($ocLazyLoad) {
                        return $ocLazyLoad.load([
                            'app/pages/deviceManagement/deviceList/detailShowCtrl.js',
                        ]);
                    }
                ]
            }
        ...
        modalInstance.result.then(function(checkedItems) {}, function(newItems) {
            ...
            if (objIdsArrQ.length == 0 && objIdsArrQS.length == 0) {
                ...
                deviceListService.queryBase($scope.neId, $scope.objids, $scope.windowId).success(function(res) {
                    if (res.data == 'sent') {
                        ...
                        messageAlertService.detailQueryFun('failed', res.msg);
                        $scope.clearBase();
                        $scope.$apply();
                    }
                })
    ...
    $scope.setBase = function(flag) {
        ...
        if (messageAlertService.sureFun('sure')) {
            ...
            if (flag == 'base') {
                ...
                if (objIdsArrQS.length == 0) {
                    ...
                    if (setValueArr.length != objIdsArrQS.length || setValueArr.length == 0 || setValueArr.indexOf(undefined) != -1 || setValueArr.indexOf('') != -1 || setValueArr.indexOf(null) != -1) {
                        // alert('please set value!')
                        ...
                        deviceListService.setBase($scope.neId, $scope.objValues, $scope.objids, $scope.qryNumber, $scope.windowId).success(function(res) {
                            if (res.data == 'send') {
                                ...
                                messageAlertService.detailSetFun('', res.msg);
                                $scope.clearBase();
                                $scope.$apply();
                            }
                        })
                ...
                if (al_objIdsArrQS.length == 0) {
                    ...
                    if (setValueArr.length != al_objIdsArrQS.length || setValueArr.length == 0 || setValueArr.indexOf(undefined) != -1 || setValueArr.indexOf('') != -1 || setValueArr.indexOf(null) != -1) {
                        // alert('please set value!')
                        messageAlertService.detailSetFun('setvalue');
                ...
                if (ra_objIdsArrQS.length == 0) {
                    ...
                    if (setValueArr.length != ra_objIdsArrQS.length || setValueArr.length == 0 || setValueArr.indexOf(undefined) != -1 || setValueArr.indexOf('') != -1 || setValueArr.indexOf(null) != -1) {
    ...
    $scope.resetBase = function() {
        if (messageAlertService.sureFun('sure')) {
            ...
            if ($scope.dtcDevicetypeclassifyid) {
                $scope.objids = '0022';
                $scope.objValues = 1;
                deviceListService.setBase($scope.neId, $scope.objValues, $scope.objids, $scope.qryNumber, $scope.windowId).success(function(res) {
                    if (res.data == 'send') {
                ...
                alert('System error, please contact the administrator!')
    ...
    function setStyle(leafId, nodes, index, path) {
        ...
        if (nodes instanceof Array) {
            ...
            for (var i = 0; i < nodes.length; i++) {
                ...
                if (nodes[i].neElement.neOnline >= 5) {
            ...
            if (nodes.neElement.tpAlarmList.length != 0) {
                ...
                // return tmpPath;
            } else if (nodes.neElement.neOnline >= 5) {
                nodes.lineStyle = {
                    color: '#5f5f5f',
                    borderColor: '#5f5f5f'
                };
    ...
    $scope.initGragh = function() {
        ...
        var option = {
            ...
            tooltip: {
                ...
                formatter: function(params) {
                    ...
                    //     `;
                    if ((params.data.neElement == null) || (!params.data.neElement)) {
                        return "No device information"
                    } else {
                        return params.name + "<br />" + params.data.neElement.neRepeaterid16 + "<br />" + params.data.neElement.neDeviceid + "<br />" + params.data.neElement.neRoute + "<br />" + params.data.neElement.neInstallplace + "<br />";
                    }
                }
            },
        ...
        document.onclick = function ff() {
            if ($scope.showContextMen == true) {
                $scope.showContextMen = false;
                $scope.$apply();
            } else if ($scope.showContextMen_Au == true) {
                $scope.showContextMen_Au = false;
                $scope.$apply();
            }
        };
