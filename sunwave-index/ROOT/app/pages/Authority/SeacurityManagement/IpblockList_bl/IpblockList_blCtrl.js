(function() {
    ...
    angular.module('SunWave.pages.Authority.SeacurityManagement.IpblockList_bl', [])
        .controller('IpblockList_blCtrl', IpblockList_blCtrl);

    function IpblockList_blCtrl($rootScope, $scope, IpblockList_blService, messageAlertService, $uibModal, $log) {

        $scope.aler = function() {
            // alert('ede')
        };

        $scope.type = "safe3"; //默认显示
        ...
        var searchForm = function() {
            ...
            $scope.clearAll();
            IpblackList_blService.search($scope.query)
        ...
        ***************************************************************/
        ...
        $scope.deleteItem = function() {
            if ($scope.checked.length == 0) {
                ...
                if (confirm("Are U Sure?")) {
                    //console.log($scope.checkedItems);
                    IpblackList_blService.deleteItem($scope.checked.toString()).success(function(response) {
        ...
        $scope.deleteItem_black = function() {
            if ($scope.checked2.length == 0) {
                ...
                if (confirm("Are U Sure?")) {
                    //console.log($scope.checkedItems2);
                    IpblackList_blService.deleteItem_black($scope.checked2.toString()).success(function(response) {
        ...
        $scope.openAddModal = function() {
            var modalInstance = $uibModal.open({
                animation: true,
                backdrop: "static",
                templateUrl: 'app/pages/Authority/SeacurityManagement/IpblockList_bl/IpModal.html',
                ...
                resolve: {
                    ...
                    IpblackList_blService: function() {
                        return IpblockList_blService;
                    },

                    deps: ['$ocLazyLoad',
                        function($ocLazyLoad) {
                            return $ocLazyLoad.load(['app/pages/Authority/SeacurityManagement/IpblockList_bl/addIpCtrl.js']);
                        }
                    ]
                }
            });
