(function() {
    ...
    angular.module('SunWave.pages.LogManagement.HWLog', [])
        .controller('HWLogCtrl', HWLogCtrl);

    function HWLogCtrl($rootScope, $scope, $log, $uibModal, HWLogService, $http, $filter) {
        $scope.query = { //查询条件
            pageIndex: 0,
            pageSize: 10,
            deviceId16: "",
        ...
        ***************************************************************/
        ...
        $scope.detailDeviceName = function(item, $event) {
            ...
            var modalInstance = $uibModal.open({
                animation: true,
                backdrop: "static",
                templateUrl: 'app/pages/LogManagement/HWLog/detailDeviceNameModal.html',
                controller: 'detailHwModalLogCtrl',
                size: "md",
                resolve: {
                    ...
                    deps: ['$ocLazyLoad',
                        function($ocLazyLoad) {
                            return $ocLazyLoad.load([
                                'app/pages/LogManagement/HWLog/detailDeviceNameModalLogCtrl.js'
                            ]);
                        }
                    ]
                }
        ...
        $scope.exportList = function(item) { 
            $rootScope.loading = true;
            return $http({    
                url: Url + '/sunwave-log-management/log/HWLog/export',
                    method: "POST",
                    headers: {      
