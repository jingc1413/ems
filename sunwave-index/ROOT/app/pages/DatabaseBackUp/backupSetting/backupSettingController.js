(function() {
    ...
    function backupSettingCtrl($scope, backupSettingService, messageAlertService) {
        ...
        $scope.submit = { //提交信息
            ...
            onlineBack: false,
            binlogBack: 0,
            path: "/home/springcloud/backpath"
        };
        ...
        backupSettingService.getUpdateBackupSetting().success(function(response) {
            ...
            if ($scope.backupSetting != null) {
                ...
                $scope.submit.backtime = $scope.backupSetting.backtime;
            }

        });

        $scope.tablesFun = function() {
            // alert($scope.content.backtables);
        };
        ...
        $scope.tables = [{
            ...
            tableName: "ne_element",
            name: "NEs Information"
        }, {
            tableName: "alm_alarmlog",
        ...
        $scope.makeExpress = function() {
            ...
            if ($scope.content.backtables != null) {
                $scope.submit.tables = $scope.content.backtables;
        ...
        $scope.getBackupSetting = function() {
            backupSettingService.getBackupSetting().success(function(res) {
                if (res) {
                    let getPeriodArr = res.period.split(' ');
                    for (let i = 0; i < getPeriodArr.length; i++) {
                        //分
                        $scope.period.minute = Number(getPeriodArr[1]);
                        //时
                        $scope.period.hour = Number(getPeriodArr[2]);
                        //周
                        $scope.period.week = Number(getPeriodArr[5]);
                    };
                    //content
                    $scope.content.backtables = res.tables;
                    //radio
        ...
        $scope.reset = function() {
            $scope.submit.backtime = '';
            $scope.content.backtables = '';
            $scope.submit.path = '';
            $scope.submit.onlineBack = false;
        };
