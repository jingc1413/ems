(function() {
    ...
    angular.module('SunWave.pages.、.ServerAlarmHistory', [])
        .controller('ServerAlarmHistoryCtrl', ServerAlarmHistoryCtrl);

    function ServerAlarmHistoryCtrl($rootScope, $scope, $uibModal, $log, $http, ServerAlarmHistoryService, messageAlertService) {
        $scope.checkbox = {
            select_all: "",
        }
        ...
        $scope.deleteItem = function() {
            if ($scope.checked.length == 0) {
                ...
                if (messageAlertService.confirmFun('sure')) {
                    // //console.log("选中项");
                    //console.log($scope.checkedItems);
                    ServerAlarmHistoryService.deleteItem($scope.checked.toString()).success(function(response) {
        ...
        var searchForm = function() {
            ...
            ServerAlarmHistoryService.searchContent($scope.query)
        ...
        $scope.exportList = function(item) { 
            $rootScope.loading = true;
            return $http({    
                url: Url + '/sunwave-alarm-management/alarm/almSystemlog3His/export',
                    method: "POST",
                    headers: {      
                ...
                    
                var filename = 'Server Alarm History.xlsx';  
        ...
        $scope.openEditDialog = function() {
            ...
            var modalInstance = $uibModal.open({
                animation: true,
                backdrop: "static",
                templateUrl: 'app/pages/AlarmManagement/ServerAlarmHistory/addModal.html',
                ...
                resolve: {
                    ...
                    deps: ['$ocLazyLoad',
                        function($ocLazyLoad) {
                            return $ocLazyLoad.load(['app/pages/AlarmManagement/ServerAlarmHistory/addModalCtrl.js']);
                        }
                    ]
                }
            });
