...
function detailAlarmNameModalCtrl($scope, transmitModalItems, currentAlarmService, currentFlag, $uibModalInstance, $filter) {
   ...
   // } 
    ...
    $scope.clearAlarm = function() {
        ...
        if ($scope.AlarmStatus == '2' || $scope.AlarmStatus == '1') {
            currentAlarmService.checkClear($scope.AlarmLogId).success(function(res) {
                if (res.data == true) {
                    currentAlarmService.alarmConfirm($scope.AlarmStatus, '4').success(function() {
                        if (res.data == true) {
